...
| 2026-09-14 | Claude Code | Empaquetar el deploy de Azure con `tar -a -c -f` en vez de `Compress-Archive` de PowerShell | `Compress-Archive` escribía las rutas internas del zip con `\` (separador de Windows); en Linux eso rompe cada archivo dentro de una subcarpeta (`rsync: failed to stat "de\Microsoft...dll"`). `tar` (incluido en Windows 10/11) siempre usa `/`, el separador que exige el estándar ZIP |
| 2026-09-14 | Claude Code | Fijar `SCM_DO_BUILD_DURING_DEPLOYMENT=false` en el App Service | El binario ya se compiló en local con `dotnet publish`; sin esta variable, Kudu trata el zip como código fuente y lo pasa por su pipeline de build/`rsync` (donde reventó el problema de separadores) en vez de solo extraerlo a `wwwroot` |
...
| 2026-09-14 | Claude Code | Recomendar `az webapp deploy --type zip` (el comando no deprecado) como forma "más confiable" de desplegar, frente a arrastrar el zip a mano | Con Basic Auth SCM ya habilitado y el zip ya corregido (estructura plana, separadores `/`), `az webapp deploy` seguía devolviendo `Kudu Status: 400` con cuerpo vacío en cada intento — tanto vía el CLI como replicando la misma llamada HTTP a mano con `curl`. La causa exacta no se determinó (posible particularidad del endpoint `/api/publish` de OneDeploy en este App Service) | Se usó el comando deprecado `az webapp deployment source config-zip`, que con el mismo zip sí completó el deploy (`status: 4` Success) y dejó la app respondiendo. Documentado en `decisions.md` como nota abierta para revisar si se retoma el despliegue manual |
...
| Despliegue en Azure App Service | No se dio el deploy por bueno con "el comando terminó sin error" (`config-zip` marcó éxito): se confirmó `status: 4` en `az webapp log deployment list`, se leyeron los logs de arranque del contenedor ("Application started") y se llamó `GET /health` contra la URL pública real, verificando conexión efectiva a Azure SQL, no solo que el proceso arrancara |
