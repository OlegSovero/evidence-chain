...
Fase: **Fases 2, 3 y 4 completas en `main` (dominio, modelo, seeder determinista, API y frontend). Backend en Azure y frontend en Vercel ya desplegados; quedan bugs de conexión entre ambos y el cierre de `/docs` antes de enviar.**
...
Azure: `evidencechain-api` (App Service Linux F1, West US 3) desplegado y respondiente — `GET /health` → `200 {"status":"healthy"}` contra Azure SQL real (`evidence_chain` en `evidencechain-sql.database.windows.net`). App Service actualmente **detenido a propósito** por Oleg (plan F1, sin costo por estar apagado; se reinicia con `az webapp start`). Ver `docs/decisions.md` §4 para el detalle completo del troubleshooting de deploy (SCM Basic Auth, separadores de ruta de `Compress-Archive`, `SCM_DO_BUILD_DURING_DEPLOYMENT`).
Vercel: frontend desplegado (Oleg, vinculando el repo de GitHub, con ayuda de otro agente Claude) en `https://evidence-chain-frontend-gsacwi4wc-evidence-chain.vercel.app`. **Bloqueador [!]**: esa URL tiene activo "Vercel Deployment Protection" (redirige con 302 a `vercel.com/sso-api`, confirmado con `curl`) — nadie sin cuenta del team de Vercel puede verla, incluidos los evaluadores. Pendiente: desactivar la protección (o limitarla a solo *Preview*) en Settings → Deployment Protection, y usar el dominio de producción estable (sin el sufijo aleatorio) tanto para compartir como para el CORS del backend.
**Bugs de integración encontrados (2026-09-15), pendientes de corregir en código:**
1. `Cors__AllowedOrigins` en Azure se configuró como el string `"[\"https://*.vercel.app\"]"` (sintaxis de array como texto plano) — las variables de entorno de .NET no parsean JSON; hace falta el formato indexado (`Cors__AllowedOrigins__0=...`).
2. Aun con el formato correcto, `policy.WithOrigins(...)` en `Program.cs` hace coincidencia exacta de string: `https://*.vercel.app` nunca calzaría contra el `Origin` real del navegador. Hace falta `SetIsOriginAllowed` o el dominio exacto de Vercel (una vez resuelto el bloqueador de arriba).
Siguiente paso: Oleg desactiva la protección de Vercel y confirma el dominio de producción; luego se corrige el CORS en `Program.cs` + la variable en Azure, se reinicia el App Service, se verifica el flujo real navegador→Vercel→Azure→Azure SQL, y se cierra `/docs` para el envío a evaluadores.
